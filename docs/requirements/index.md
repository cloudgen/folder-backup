...
| requirement-shell-cli-interface | Shell CLI interface (commands, flags, dispatch, modes); submit `--add`/`--update`; **generate-sudoer-request**; **`menu`/`main` routed**; **case 2 TTY empty argv = menu**; test-purpose grant-emit listed apart; grant/draft verbs live; family **sudoers** not dispatched | shell | Active (1.8.0) | `requirement-shell-cli-interface.md` | 2026-09-03 |
...
| requirement-shell-cli-default-interaction | Claimed TTY numbered list on **empty argv** (case 2) and `menu`/`main`; daily work + family **sudoers** submenu; default CLI main menu style; grant/draft setup verbs live; **implemented** | shell | Active (1.5.0) | `requirement-shell-cli-default-interaction.md` | 2026-09-03 |
...
| requirement-domain-folder-backup | Domain **surface** SSOT (four pillars); ops defer to folder-archive-backup; submit public inbound; **host-probe add/update**; **independent generate-sudoer-request**; grant-emit **test-purpose** (help apart); sudoers submenu | domain | Active (1.6.3) | `requirement-domain-folder-backup.md` | 2026-09-03 |
