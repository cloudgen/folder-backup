...
| Box | Meaning | Example |
|-----|---------|---------|
| You / this login | Read `[INFO]` / `[ERROR]` / JSON on the terminal | `folder-backup version` |
| The other role | Scripts parse `--json` without human banners | `folder-backup --json version` |
...
|---------|---------------|----------|
| `./src/folder-backup` | ship unit | `out_*` / `util_app_ident` |
| `folder-backup --json help` | command | one JSON object on stdout |

| You do… | What it means | What you type |
|---------|---------------|---------------|
| Open the numbered list | Header names **folder-backup**(*live version*); descriptions after the colon are italic and light gray on a real terminal | `folder-backup` |
| Pipe the same command | No colors, no slant | `folder-backup </dev/null` |
...
| `util_app_ident` | Identity token **bold** name / *italic* version (capture into `out_*`) | pipeline (class B) | n/a | Plain `folder-backup(VERSION)` when `TTY=0` or `JSON=1` |
...
| MUST | MUST NOT |
|------|----------|
| Main-menu / APP_NAME-led submenu headers; about identity titles | Usage / `Next:` invocation lines (`folder-backup <verb>`) |
| Live Config `APP_NAME` and `VERSION` (same scalars as `version`) | A bare `folder-backup` on that header |
| TTY: bold name, italic version; off-TTY: plain `folder-backup(VERSION)` | CSI when `TTY=0` or `JSON=1`; raw CSI in README fences |
| Still via `out_*` (quiet/JSON rules unchanged) | Glue the token into JSON as the only `app`/`version` fields |

Proof when the numbered menu is claimed: **TP-CLI-18** (product alias of portable **TP-CLI-17**).
...
| **TP-CLI-18** | same | **have** — identity token + numbered-row ink (AC-5) |
